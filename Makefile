...
#-----------------------------------------------------------------------------
vendor_remove_dirs := data-app etc/precust_theme media/lockscreen media/theme media/wallpaper etc/WhetstonePermission
...
#-----------------------------------------------------------------------------
vendor_saved_apps := Bluetooth BluetoothMidiService BookmarkProvider MiuiBluetooth EasterEgg ExtShared HTMLViewer KeyChain PacProcessor PrintRecommendationService PrintSpooler Stk UserDictionaryProvider WAPPushManager \
			miuisystem telresources QtiTelephonyService qcrilmsgtunnel SecurityCoreAdd TimeService shutdownlistener datastatusnotification embms \
			BackupRestoreConfirmation BlockedNumberProvider CellBroadcastReceiver DefaultContainerService EmergencyInfo ExternalStorageProvider FusedLocation PicoTts NQNfcNci InputDevices \
			ManagedProvisioning MtpDocumentsProvider ProxyHandler SharedStorageBackup Shell StatementService StorageManager com.qualcomm.location QtiTetherService CNEService miui MiuiContentCatcher \
			MiuiCamera
...
#-----------------------------------------------------------------------------
board_modify_apps := ConnectivitySettings Telecom TeleService
...
override_property += \
    ro.flyme.romer=ry09iu \
    ro.product.model_romer=mi6_ry09iu
